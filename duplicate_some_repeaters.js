...
async function duplicateDocuments() {
  for (const docId of documentIds) {
    try {
      const snapshot = await db.collection('repeaters').doc(docId).get();
      if (snapshot.exists) {
        const data = snapshot.data();
        await db.collection('repeaters').doc().set(data);
        console.log(`Document ${docId} duplicated successfully.`);
      } else {
        console.log(`Document ${docId} does not exist.`);
      }
    } catch (error) {
      console.error(`Error duplicating document ${docId}:`, error);
    }
  }
}
