...
// Configuration
const CSV_FILE_PATH = path.join(__dirname, 'cached_centres_to_import.csv');
...
function transformRowToDocument(row) {
    ...
    // Add centre_image if provided
    if (row['Image'] && row['Image'].trim() !== '') {
        data.centre_image = row['Image'].trim();
    }

    // Override sports if provided (comma-separated)
    if (row['Sports'] && row['Sports'].trim() !== '') {
        data.sports = row['Sports'].split(',').map(s => s.trim().toLowerCase());
    }
 ...
 */
async function importCachedCentres() {
    console.log('[START] Cached Centres Import Process');
    console.log('============================================================');
    ...
    console.log('  - Latitude (required)');
    console.log('  - Longitude (required)');
    console.log('  - Image (optional)');
    console.log('  - Sports (optional, comma-separated, defaults to "soccer")');
    console.log('============================================================');
    console.log('');
        ...
        fs.createReadStream(CSV_FILE_PATH)
            ...
            .on('data', (row) => {
                ...
                try {
                    ...
                    const hasImage = document.data.centre_image ? ' [has image]' : '';
                    console.log(`[OK] Row ${rowNumber}: ${document.data.centre_name} (${document.docId})${hasImage}`);
                } catch (error) {
