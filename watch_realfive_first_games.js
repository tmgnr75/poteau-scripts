/**
 * Watch RealFive Pont-à-Mousson for its first published games, and say so on Slack.
 *
 * RealFive got its Poteau Max account on 2026-09-14. Unlike LE PARK Servon,
 * which was handed a live market, this venue is cold: 0 games ever organized
 * within 15 km, 0 alerts on the place_id, and none of the 48 players whose
 * availabilities reach it has ever played a game on Poteau.
 *
 * That makes this watch MORE useful than Le Park's, not less. There is no
 * other signal: no player traffic at the venue to look at, and the daily
 * health report does not track individual centres. Whether Fares publishes is
 * the only thing that will tell us if this partnership took.
 *
 * WHAT COUNTS AS "HIS" GAME
 * -------------------------
 * Only games whose `organizer` is the centre's uid. Filtering on place_id
 * would be harmless today (nobody organizes there) but wrong the moment a
 * player does, which is exactly the outcome we are hoping for.
 *
 * SELF-TERMINATING
 * ----------------
 ...
 *
 * Run:
 *   node watch_realfive_first_games.js           check, print, no Slack
 *   SLACK=1 node watch_realfive_first_games.js   post to #health-reports
 *   node watch_realfive_first_games.js --status  show state and exit
 *   node watch_realfive_first_games.js --reset   clear state (start over)
 *
 * Scheduled hourly by com.poteau.realfivewatch.plist.
 */
...
const UID = process.env.REALFIVE_UID || 'W514nnZZkwR1XVX666uDzTnvunY2';
const CENTRE_NAME = 'RealFive Pont-à-Mousson';
const PLACE_ID = 'ChIJ6zeXoQXHlEcRJOm0i4shoAg';
const STATE_FILE = path.join(__dirname, '.realfive_watch_state.json');
const ONBOARDED = '2026-09-14';
...
async function main() {
    ...
    if (UID === '__UID__') {
        console.error('UID placeholder not replaced. Set REALFIVE_UID or edit the constant.');
        process.exit(1);
    }

    // Games the CENTRE published, never place_id.
    ...
    console.log(
        `${CENTRE_NAME}: ${games.length} game(s) published by the centre, ${fresh.length} new since last check.`
    );
    ...
    if (!state.firstGameAnnounced && daysSince >= DEADLINE_DAYS) {
        // Le Park's version contrasted the centre's silence with players still
        // organizing there. Here that contrast does not exist yet, so report
        // the honest version: whether anything at all happened at the venue.
        ...
        const byPlayersLine =
            n === 0
                ? "Aucun match non plus du côté des joueurs, le centre n'a jamais démarré sur Poteau."
                : n === 1
                  ? '1 match a quand même été organisé chez lui par un joueur.'
                  : `${n} matchs ont quand même été organisés chez lui par des joueurs.`;
